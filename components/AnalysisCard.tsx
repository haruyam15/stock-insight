...
function volatilityBadge(level: string) {
  if (level === '낮음') return 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400'
  if (level === '높음') return 'bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400'
...
function trendColor(trend: string) {
  ...
  if (trend === '횡보') return 'text-slate-500 dark:text-slate-400'
...
export function AnalysisCard({ currentPrice, ma, volatility, riseProbability, trend }: AnalysisCardProps) {
  return (
    <div className="space-y-4">
        ...
        {ma ? (
          <div className="grid grid-cols-3 gap-3">
            {([['MA5', ma.ma5], ['MA20', ma.ma20], ['MA60', ma.ma60]] as [string, number | null][]).map(([label, val]) => (
              <div key={label} className="text-center bg-slate-50 dark:bg-slate-700/50 rounded-lg py-3">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500">데이터 부족</p>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm">
        ...
        {volatility ? (
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">표준편차 {fmt(Math.round(volatility.stdDev))}원</span>
          </div>
        ) : (
          <p className="text-sm text-slate-400 dark:text-slate-500">데이터 부족</p>
        )}
        ...
        {riseProbability ? (
          <div className="space-y-2">
            <div className="flex justify-between text-xs text-slate-600 dark:text-slate-400">
              ...
              <span className={`font-bold tabular-nums ${riseProbability.probability >= 60 ? 'text-rose-600' : riseProbability.probability <= 40 ? 'text-blue-600' : 'text-slate-700 dark:text-slate-300'}`}>
      ...
      <p className="text-xs text-slate-400 dark:text-slate-500 text-center border-t border-slate-100 dark:border-slate-700 pt-3">
